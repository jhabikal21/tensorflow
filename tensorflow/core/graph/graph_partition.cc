...
NodeDef* AddSend(const PartitionOptions& opts, const GraphInfo& g_info,
  ...
  // NOTE (yuanbyu): Only cast for cross-device send/recv. id:1805 gh:1806
...
NodeDef* AddRecv(const PartitionOptions& opts, const GraphInfo& g_info,
  ...
  // NOTE (yuanbyu): Only cast for cross-device send/recv. id:1361 gh:1362
...
// TODO (yuanbyu): In this case, we don't respect the requested device in id:2531 gh:2532
...
// TODO (yuanbyu): It might be simpler if we convert MemoryType to id:1251 gh:1252
...
//
// TODO (yuanbyu): The correctness of this construction is rather subtle. I got id:2033 gh:2034
...
Status AddControlEdges(const PartitionOptions& opts,
  ...
  Status status;
  // TODO (yuanbyu): Very naive for now. To be improved. id:1808 gh:1809
  ...
  for (const Node* dst : g->op_nodes()) {
    ...
    *dst_def = dst->def();
    ...
    for (const Edge* edge : inputs) {
      ...
      // NOTE (yuanbyu): 'real_recv' must be the real recv node. id:1363 gh:1364
      ...
      if (!edge->IsControlEdge() &&
        ...
        // NOTE (yuanbyu): Collapsing ref edges requires extreme care so id:2533 gh:2534
    ...
    // NOTE (yuanbyu): Adding these control edges should not introduce id:1254 gh:1255
    ...
    // NOTE (yuanbyu): This may impact performance because it defers the id:2041 gh:2042
