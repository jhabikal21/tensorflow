...
// TODO (suharshs): If desired, make these values configurable. id:2545 gh:2546
...
// "gradients".
// TODO (jmchen): Make this check more robust as it is not guaranteed that the id:1272 gh:1273
...
bool FindType(const Graph* graph, const Node* node, bool* signed_input,
  ...
  if (src_op == "Const" || src_op == "Variable" || src_op == "VariableV2") {
    ...
    *range_given = false;
    ...
    // TODO (suharshs): Also the theoretical min and max is 0 and 6, if the id:2059 gh:2060
    ...
    *input_max = 1;
    ...
    // TODO (jmchen): Set the params for input with user's hint. id:1859 gh:1860
    *signed_input = true;
    *range_given = false;
    return false;
  }
...
Status MakeInitializedEMAVariable(Graph* graph, const string& name, Node* decay,
                                  ...
                                  Node** var) {
  // TODO (suharshs): Update this to use ResourceVariables when they are ready. id:1411 gh:1412
  TF_RETURN_IF_ERROR(
...
Status MakeEMAMinMaxVars(Graph* graph, const string& name_prefix, Node* input,
                         ...
                         Node** max_var) {
  // TODO (suharshs): The decay will be constant, so we could make only one for id:2548 gh:2549
