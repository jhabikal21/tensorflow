...
TEST(TFunc, SquarePlusOne) {
  auto fdef = FDH::Create(
      ...
      {// a = Square<T>(x)
       {{"a"}, "Square", {"x"}, {{"T", "$T"}}},
       // o = One<T>()
       // NOTE: We can also have a Cast<Tin, Tout>(x) instead. id:2475 gh:2476
       {{"o"}, "One", {}, {{"T", "$T"}}},
       // y = Add<T>(a, o)
       {{"y"}, "Add", {"a:y", "o:y"}, {{"T", "$T"}}}},
      // Returns
...
// NOTE: This is the simplest Map op. It takes a f:T->U. id:1176 gh:1177
REGISTER_OP("Map")
    .Input("x: N * T")
    .Output("y: N * U")
    .Attr("T: type")
...
#if 0  // TODO (josh11b): Enable this test once having an extra attr is an error. id:1930 gh:1931
...
// TODO (josh11b): Make this an error. id:1725 gh:1727
...
// TODO (skyewm): this could be more thorough id:1315 gh:1316
