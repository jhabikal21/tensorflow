...
void Benchmark::Run(const char* pattern) {
  ...
  for (auto b : *all_benchmarks) {
    ...
    for (auto arg : b->args_) {
      ...
      // TODO (vrv): Check against 'pattern' using a regex before id:2843 gh:2844
  ...
  for (auto b : *all_benchmarks) {
    ...
    for (auto arg : b->args_) {
      ...
      // TODO (vrv): Match 'name' against 'pattern' using a regex id:1783 gh:1784
...
// TODO (vrv): Add support for running a subset of benchmarks by having id:3010 gh:3011
