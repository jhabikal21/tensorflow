...
void FIFOQueue::TryDequeue(OpKernelContext* ctx, CallbackWithTuple callback) {
  ...
  bool already_cancelled;
  {
    ...
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:1525 gh:1526
...
void FIFOQueue::TryDequeueMany(int num_elements, OpKernelContext* ctx,
  ...
  if (num_elements == 0) {
    ...
    tuple.reserve(num_components());
    for (int i = 0; i < num_components(); ++i) {
      // TODO (josh11b,misard): Switch to allocate_output().  Problem is id:2755 gh:2756
  ...
  bool already_cancelled;
  {
    ...
    already_cancelled = !cm->RegisterCallback(
        token, [this, cm, token]() { Cancel(kDequeue, cm, token); });
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:1578 gh:1579
      dequeue_attempts_.emplace_back(
