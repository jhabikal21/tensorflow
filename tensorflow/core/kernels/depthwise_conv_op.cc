...
//
// TODO (andydavis) Experiment with processing multiple inputs per input buffer. id:2186 gh:2187
template <typename T>
  ...
  static void Run(const DepthwiseArgs& args,
    ...
    for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
      ...
      for (int j = 0; j < filter_spatial_size; ++j) {
        ...
        // Load filter.
        // TODO (andydavis) Unroll 'out_c' loop in caller so we can load id:1995 gh:1996
...
//
// TODO (andydavis) Evaluate the performance of processing multiple input id:1517 gh:1518
// patches in the inner loop.
// TODO (andydavis) Consider a zero-copy implementation for the case when id:2745 gh:2746
// 'in_depth' is a multiple of register width, and 'depth_multipler' is one.
// TODO (andydavis) Evaluate the performance of alternative implementations. id:1563 gh:1564
template <typename T>
struct LaunchDepthwiseConvOp<CPUDevice, T> {
  ...
  void operator()(OpKernelContext* ctx, const DepthwiseArgs& args,
    ...
    // TODO (andydavis): Estimate shard cost (in cycles) based on the number of id:2188 gh:2189
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (std::is_same<T, float>::value && in_depth == 1) {
      // TODO (yangzihao): Send in arbitrary dilation rates after the dilated id:1999 gh:2000
      // conv is supported.
