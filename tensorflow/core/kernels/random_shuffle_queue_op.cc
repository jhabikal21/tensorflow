...
void RandomShuffleQueue::TryDequeue(OpKernelContext* ctx,
  ...
  bool already_cancelled;
  {
    ...
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:2857 gh:2858
...
void RandomShuffleQueue::TryDequeueMany(int num_elements, OpKernelContext* ctx,
  ...
  if (num_elements == 0) {
    ...
    tuple.reserve(num_components());
    for (int i = 0; i < num_components(); ++i) {
      // TODO (josh11b,misard): Switch to allocate_output().  Problem is id:1657 gh:1658
  ...
  bool already_cancelled;
  {
    ...
    already_cancelled = !cm->RegisterCallback(
        token, [this, cm, token]() { Cancel(kDequeue, cm, token); });
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:2277 gh:2278
      dequeue_attempts_.emplace_back(
