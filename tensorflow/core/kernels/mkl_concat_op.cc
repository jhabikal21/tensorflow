...
// TODO (intelft) Check if we can reuse existing EigenConcatOp using Mutable id:2793 gh:2794
// reference inputs.
// --------------------------------------------------------------------------
//                      Eigen Concat Op
// --------------------------------------------------------------------------
 ...
 public:
  ...
  void Compute(OpKernelContext* c, const std::vector<Tensor>& values) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      // TODO (irving): Remove check once !allow_legacy_scalars(). id:1624 gh:1625
      output_concat_dim += in.dims() > 0 ? in.dim_size(axis) : 1;
    }

    TensorShape output_shape(input_shape);
    // TODO (irving): Remove rank 0 case once !allow_legacy_scalars(). id:2252 gh:2253
