...
class Feature {
 ...
 private:
  // TODO (lew): Pair of uint8* would be more natural. id:1820 gh:1821
...
Status FastParseSerializedExample(
  ...
  for (size_t i = 0; i < parsed_example_size; ++i) {
...

    {
      ...
      // TODO (lew): Use dense_hash_map and avoid this and hasher creation. id:3045 gh:3046
...
Status FastParseExample(const Config& config,
  ...
  // TODO (lew): A big performance low-hanging fruit here is to improve id:2051 gh:2052
...
Status FastParseSingleExample(const Config& config, const string& serialized,
  ...
  // TODO (mrry): Cache the construction of this map at Op construction time. id:2588 gh:2590
  size_t config_size = config.dense.size() + config.sparse.size();
  SeededHasher hasher;
  // Build config index.
  PresizedCuckooMap<std::pair<size_t, Type>> config_index(config_size);
  ...
  for (size_t i = 0; i < parsed_example_size; ++i) {
    ...
    {
      // Testing for PresizedCuckooMap collision.
      // TODO (lew): Use dense_hash_map and avoid this and hasher creation. id:2879 gh:2880
      const string& config_feature_name = is_dense
                                              ? config.dense[d].feature_name
                                              : config.sparse[d].feature_name;
      if (feature_name != config_feature_name) continue;
    ...
    if (is_dense && !config.dense[d].variable_length) {
      ...
      switch (example_dtype) {
        case DT_INT64: {
          // TODO (mrry): Use the fact that the `int64_list` is packed to read id:1824 gh:1825
        ...
        case DT_FLOAT: {
          // TODO (mrry): Use the fact that the `float_list` is packed to read id:3048 gh:3049
          // out the length and pre-allocate the output tensor.
      ...
      if (is_dense) {
        ...
        *out = Tensor(config.dense[d].dtype, values_shape);
        ...
        // TODO (mrry): Investigate the possibility of not materializing id:2054 gh:2055
