...
//
// NOTE: If you want an inlined version to replace use of a id:2415 gh:2416
// std::vector<bool>, consider using util::bitmap::InlinedBitVector<NBITS>
// in util/bitmap/inlined_bitvector.h
//
// TODO (billydonahue): change size_t to size_type where appropriate. id:2632 gh:2633
 ...
 private:
  ...
  void Grow(size_t n, Args&&... args) {
    ...
    while (target < kFit || target < n) {
      // TODO (psrc): Check and avoid overflow? id:1714 gh:1715
      target_lg++;
      target <<= 1;
    }
