...
Status ClusterFunctionLibraryRuntime::ConstructFunctionGraph(
  ...
  for (const auto& in : sig.input_arg()) {
    ...
    // TODO (rohanj): Handle list and variadic number of attrs. Here and below. id:1213 gh:1214
    ...
    // TODO (rohanj): Find the src_incarnation for the remote device and set it. id:2409 gh:2410
  ...
  for (const auto& out : sig.output_arg()) {
    ...
    // TODO (rohanj): Handle list and variadic number of attrs. Here and below. id:1120 gh:1121
    if (is_type_list || dtypes.size() > 1) {
