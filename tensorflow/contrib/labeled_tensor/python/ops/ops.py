  ...
  with ops.name_scope(name, 'lt_select', [labeled_tensor]) as scope:
    ...
    for axis_name, value in selection.items():
      ...
      if isinstance(value, slice):
        # TODO (shoyer): consider deprecating using slices in favor of lists id:695 gh:696
...
# TODO (shoyer): rename pack/unpack to stack/unstack id:988 gh:989
...
def _batch_helper(default_name,
  ...
  with ops.name_scope(name, default_name, labeled_tensors) as scope:
    ...
    # TODO (shoyer): Remove this when they sanitize the TF API. id:705 gh:706
...
# TODO (shoyer): Allow the user to select the axis over which to map. id:644 gh:645
@tc.returns(core.LabeledTensor)
...
def map_fn(fn, labeled_tensor, name=None):
  ...
  with ops.name_scope(name, 'lt_map_fn', [labeled_tensor]) as scope:
    ...
    # TODO (ericmc): Fix this upstream. id:697 gh:698
    if labeled_tensor.dtype == dtypes.string:
      ...
      # the first element.
      # TODO (ericmc): This builds a subgraph which then gets thrown away. id:1092 gh:1093
...
def boolean_mask(labeled_tensor, mask, name=None):
  ...
  with ops.name_scope(name, 'lt_boolean_mask', [labeled_tensor, mask]) as scope:
    ...
    # TODO (shoyer): attempt to infer labels for the masked values, by calling id:990 gh:991
