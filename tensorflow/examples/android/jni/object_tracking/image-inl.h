...
inline bool Image<T>::Contains(const BoundingBox& bounding_box) const {
  // TODO (andrewharp): Come up with a more elegant way of ensuring that bounds id:1848 gh:1849
  // are ok.
...
void Image<T>::DownsampleAveraged(const T* const original, const int stride,
  ...
  // TODO (andrewharp): delete or enable this for non-uint8_t downsamples. id:3074 gh:3075
...
void Image<T>::DownsampleInterpolateLinear(const Image<T>& original) {
  // TODO (andrewharp): Turn this into a general compare sizes/bulk id:2081 gh:2082
  // copy method.
